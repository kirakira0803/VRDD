...
function initFields(defaultFields = null) {
	...
	if (fields && Array.isArray(fields)) {
		for (let k = 0; k < fields.length; k++) {
			...
			if (!helper.isDefined(curForm['title'])) curForm['title'] = 'VRChat ID';
			...
			if (!helper.isDefined(curForm['edit'])) curForm['edit'] = true; //是否可编辑删除本字段
			if (!helper.isDefined(curForm['disabled'])) curForm['disabled'] = false; //是否可修改
...
function getMustHint(type) {
	if (type == 'image' || type == 'file') return '请上传';
	...
	for (let k = 0; k < fields.length; k++) {
		...
		if (val.length > 0 && (type == 'file')) {
			if (fields[k].max == fields[k].min) {
				let len = fields[k].max;
				if (val.length != len) {
					fields[k].focus = title + ' 个数必须为' + len + '个';
					pageHelper.anchor('form' + forms[k].mark, that);
					return pageHelper.showModal(title + ' 个数必须为' + len + '个');
				}
			}
			else {
				if (val.length > fields[k].max) {
					fields[k].focus = title + ' 个数不能多于' + fields[k].max + '个';
					pageHelper.anchor('form' + forms[k].mark, that);
					return pageHelper.showModal(title + ' 个数不能多于' + fields[k].max + '个');
				}

				if (val.length < fields[k].min) {
					fields[k].focus = title + ' 个数不能少于' + fields[k].min + '个';
					pageHelper.anchor('form' + forms[k].mark, that);
					return pageHelper.showModal(title + ' 个数不能少于' + fields[k].min + '个');
				}
			}
		}
